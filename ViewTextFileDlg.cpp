...
BOOL CViewTextFileDlg::OnInitDialog()
{
	...
	try
	{
		...
		// create buffer with file's content
		...
		if (nFileLength > 0)
		{
			...
			if (NULL != pFileBuffer)
			{
				...
				const std::string pUTF8(pFileBuffer);
				const std::wstring pUnicode = utf8_to_wstring(pUTF8);
				CString strConvertedText(pUnicode.c_str());
				// actual show the content of file
				m_ctrlTextFile.SetWindowText(strConvertedText);
				// delete temporar buffer
				delete pFileBuffer;
				pFileBuffer = NULL;
			}
		}
...
BOOL CViewTextFileDlg::DestroyWindow()
{
	RECT pWndRect;
