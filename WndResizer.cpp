...
int CWndResizer::CSplitContainer::GetSplitterSize(CPanel * m_pPanelAA, CPanel * m_pPanelBB)
{
	if (m_Orientation == CWndResizer::SPLIT_CONTAINER_H)
	{
		int nWidth = m_pPanelBB->left - m_pPanelAA->right;
		return nWidth;
	}
	else // vertical
	{
		int nHeight = m_pPanelBB->top - m_pPanelAA->bottom;
		return nHeight;
	}
}
...
CWndResizer::CSpitterPanel::CSpitterPanel(const CRect * prc, CWndResizer::SplitterOrientation type) : CPanel(prc)
{
	...
	if (m_pGrippePanel == NULL)
	{
		...
		CSize pSize(2, 2);
		m_pGrippePanel->SetMinSize(pSize);
